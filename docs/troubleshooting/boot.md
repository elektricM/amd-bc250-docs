...
# DRM devices
ls -la /sys/class/drm/
# Should show card1, card1-DP-1 (or card0 on some systems)
...
A: Check if system updated kernel. Run `uname -r` from live USB after mounting system drive. If 6.15.0-6.15.6 or 6.17.8-6.17.10, that's the problem.
...
A: BC-250 works best with 6.18.18 LTS (current recommendation), 6.19.x (confirmed working), or 6.17.11+. Kernels 6.15.0-6.15.6 and 6.17.8-6.17.10 break GPU drivers. Avoid 6.10 and below (too old).
...
A: Yes! Avoid kernel 6.15.0-6.15.6 and 6.17.8-6.17.10 (GPU driver broken). Use 6.18.18 LTS or 6.19.x (both confirmed working). Keep Mesa 25.1+, disable IOMMU in BIOS, always clear CMOS after BIOS flash.
